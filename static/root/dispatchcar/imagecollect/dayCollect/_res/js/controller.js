var app = angular.module('dayCollect', ['toastr']);
app.controller('dayCollectCtrl', function($scope,imageSer,toastr){
	imageSer.dayCollect().then(function(response){
    ...
    $scope.collect = function() {
        ...
        imageSer.dayCollect(data).then(function(response){
            if(response.data.code == 0){
                $scope.figureWeeks = response.data.data;
                var myChart = echarts.init(document.getElementById('monthImg'));
                var option2=$scope.figureWeeks;
