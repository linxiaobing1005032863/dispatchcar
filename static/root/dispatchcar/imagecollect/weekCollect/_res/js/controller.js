var app = angular.module('weekCollect', ['toastr']);
app.controller('weekCollectCtrl', function($scope,imageSer,toastr){
	  imageSer.weekCollect().then(function(response){
  ...
  /*  //月汇总   月租车协议图形化汇总
    ...
    $scope.collect = function() {
        ...
        imageSer.weekCollect(data).then(function(response){
            if(response.data.code == 0){
                $scope.figureWeeks = response.data.data;
                var myChart = echarts.init(document.getElementById('monthImg'));
                var option2=$scope.figureWeeks;
