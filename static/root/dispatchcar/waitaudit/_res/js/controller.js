...
app.controller('waitModuleCtrl',function ($scope,$state) {
    ...
    $scope.$on("getId3", function(event, msg){
        $scope.idListd3 = msg;
    });
    ...
    $scope.budget = function(){
        if($scope.idListd1){
            $state.go('root.dispatchcar.waitaudit.budget[12]',{id:$scope.idListd1,page:$scope.page});
            $scope.menuClass = 'budgetMenu'
        }
    };
    $scope.client = function(){
        if($scope.idListd3){
            $state.go('root.dispatchcar.waitaudit.client[12]',{id:$scope.idListd3,page:$scope.page});
            $scope.menuClass = 'clientMenu'
        }
    };
    $scope.head = function(){
        if($scope.idListd2){
            $state.go('root.dispatchcar.waitaudit.head[12]',{id:$scope.idListd2,page:$scope.page});
